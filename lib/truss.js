...
/***/ function(module, exports, __webpack_require__) {
	...
	var TrussLayout = function () {
	   ...
	   */
							...
							if (sliderProps.newY) {
								...
								if (!sliderProps.dimensions) {
									var dim = sliderProps.dimensions = {},
									    _props = self._resize(1, i, false),
									    props2 = self._resize(-1, i, false);
	
									dim.y1 = _props[2].y === Infinity ? sliderProps.positive = self.gridConf.hMax : sliderProps.positive = _props[2].y;
								...
								if (slider[len - 1] && slider[len - 1].isVertical === false && !slider[len - 1].dimensions) {
									var _sliderProps = slider[len - 1],
									    ...
									    _props2 = self._resize(1, i - 1, false),
									    _props3 = self._resize(-1, i - 1, false);
	
									_dim.y1 = _props2[2].y === Infinity ? _sliderProps.positive = self.gridConf.hMax : _sliderProps.positive = _props2[2].y;
									_dim.y2 = _props3[2].y === Infinity ? _sliderProps.positive = self.gridConf.hMax : _sliderProps.positive = _props3[2].y;
								}
	
								if (slider[len + 1] && slider[len + 1].isVertical === false && !slider[len + 1].dimensions) {
									var _sliderProps2 = slider[len + 1],
									    _dim2 = _sliderProps2.dimensions = {},
									    _props4 = self._resize(1, i + 1, false),
									    _props5 = self._resize(-1, i + 1, false);
	
									_dim2.y1 = _props4[2].y === Infinity ? _sliderProps2.positive = self.gridConf.hMax : _sliderProps2.positive = _props4[2].y;
									_dim2.y2 = _props5[2].y === Infinity ? _sliderProps2.positive = self.gridConf.hMax : _sliderProps2.positive = _props5[2].y;
								}
	     	...
	     	add margin value at time of creation */
						...
						elem.addEventListener('mouseup', function () {
							...
							if (sliderProps.currX) {
								// if newx is greater the zero
								var props = self._resize(sliderProps.currX - sliderProps.newX, i, true);
			...
			key: '_getMinMoveValue',
			value: function _getMinMoveValue(side) {}
		}]);
