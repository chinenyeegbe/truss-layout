				...
				if (c > 1) {
					...
					var _loop2 = function _loop2(i) {
						var startWidth = Array.isArray(cw) ? cw[i - 1] - margin : cw,
						    left = startWidth * i + _sliderHeight * (i - 1);
	...
	var Grid = function () {
	   ...
	   */
			...
			value: function resizeInnerContainers(isvertical, change, operation) {
				...
				var op = property === 'height' ? operation : 1;
				...
				for (var _key in sliders) {
					if (sliders.hasOwnProperty(_key)) {
						this.changeSliderDimension(sliders[_key], isvertical, amount, _key, op);
					}
				}
			...
			value: function changeSliderDimension(sliderObj, isVertical, amount, key, op) {
